package Higher_Order_Methods

case class TempData(day: Int,doy:Int, month:Int, year:Int,precip:Double,snow: Double, trav:Double,tmax:Double,tmin:Double)
object TempData {

  def toDoubleOrNeg(s:String): Double = {
    try{
      s.toDouble
    } catch {
      case _:NumberFormatException => -1
    }
  }
  def main(args: Array[String]): Unit = {
    val source = scala.io.Source.fromFile("C:\\Spark_Problems\\BigDataAnalyticswithSpark-master\\MN212142_9392.csv")
    val lines = source.getLines().drop(1)
    /*
    val SparkTest0331.data = lines.map{line =>
      val p = line.split(",")
        val dummyRDD = rawRDD.map(_.split("\\,")).map(p => (p(0).toInt, p(1)toInt, p(2), p(3), p(4), p(5)))
      */
    /* val SparkTest0331.data = lines.filterNot(_.contains(",.,")).map { line => */
    val data = lines.flatMap{ line =>
      val p = line.split(",")
      if(p(6) == "." || p(7) =="." || p(8) ==".") Seq.empty else
      Seq(TempData(p(0).toInt,p(1).toInt,p(2).toInt,p(3).toInt,toDoubleOrNeg(p(4)),toDoubleOrNeg(p(5)),p(6).toDouble,p(7).toDouble, p(8).toDouble))
    }.toArray
    source.close()
    println(data.length)
    val maxTemp = data.map(_.tmax).max
    val hotDays = data.filter(_.tmax == maxTemp)
    println(s"Hot Days are ${hotDays.mkString(", ")}")

    val hotDay = data.maxBy(_.tmax)
    println(s"Hot Day 1 is $hotDay")

    val hotDay2 = data.reduceLeft((d1,d2) => if(d1.tmax >= d2.tmax) d1 else d2)
    println(s"Hot Day 2 is $hotDay2")

    val rainyCount = data.count(_.precip >= 1.0)
    println(s"There are $rainyCount rainy days. ${rainyCount*100.0/data.length} persent")

    val (rainySum,rainyCount2) = data.foldLeft(0.0 -> 0)
    {
      case ((sum,cnt),td) =>
     if(td.precip < 1.0) (sum,cnt) else (sum+td.tmax,cnt+1)
  }
    println(s"Average Rainly SparkTest0331.temp is ${rainySum/rainyCount2}")

    val monthGroups = data.groupBy(_.month)
    val monthlyTemp = monthGroups.map{ case (m,days) =>
    m -> days.foldLeft(0.0) ((sum,td) => sum+td.tmax)/days.length}
    monthlyTemp.toSeq.sortBy(_._1).foreach(println)
    data.take(5).foreach(println)
  }

}
